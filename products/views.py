...
def categories_add_view(request):
    ...
    if request.method == 'POST':
        ...
        if Category.objects.filter(**attributes).exists():
            messages.error(request, 'Category Sudah Ada',
                           extra_tags="warning")
        ...
        try:
            ...
            messages.success(request, 'Kategori: ' +
                             attributes["name"] + ' Tersimpan Sukses', extra_tags="success")
            return redirect('products:categories_list')
        except Exception as e:
            messages.success(
                request, 'Terjadi Error Membuat Proses!', extra_tags="danger")
            print(e)
            return redirect('products:categories_add')
...
def categories_delete_view(request, category_id):
    ...
    try:
        ...
        category = Category.objects.get(id=category_id)
        category.delete()
        messages.success(request, 'Kategori: ' + category.name +
                         ' Terhapus!', extra_tags="success")
        return redirect('products:categories_list')
    except Exception as e:
        messages.success(
            request, 'There was an error during the elimination!', extra_tags="danger")
...
def products_add_view(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, 'Produk: ' +
                             attributes["name"] + ' Behasil Dibuat!', extra_tags="success")
...
def products_update_view(request, product_id):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, 'Produk: ' + product.name +
                             ' Berhasil Diperbaruhi!', extra_tags="success")
            return redirect('products:products_list')
        except Exception as e:
            messages.success(
