...
def customers_add_view(request):
    ...
    if request.method == 'POST':
        ...
        if Customer.objects.filter(**attributes).exists():
            messages.error(request, 'Pelanggan Sudah Ada!',
                           extra_tags="warning")
        ...
        try:
            ...
            messages.success(request, 'Pelanggan: ' + attributes["first_name"] + " " +
                             attributes["last_name"] + ' Berhasil Dibuat!', extra_tags="success")
...
def customers_update_view(request, customer_id):
    ...
    if request.method == 'POST':
        try:
            ...
            messages.success(request, 'Pelanggan: ' + customer.get_full_name() +
                             ' Berhasil Di Update!', extra_tags="success")
            return redirect('customers:customers_list')
        except Exception as e:
            messages.success(
...
def customers_delete_view(request, customer_id):
    ...
    try:
        ...
        customer.delete()
        messages.success(request, 'Pelanggan: ' + customer.get_full_name() +
                         ' Berhasil Di Hapus!', extra_tags="success")
        return redirect('customers:customers_list')
    except Exception as e:
        messages.success(
